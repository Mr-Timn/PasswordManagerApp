...
**************************************************/
static void pack_pk(uint8_t r[KYBER1024_INDCPA_PUBLICKEYBYTES],
                    polyvec *pk,
                    const uint8_t seed[KYBER1024_SYMBYTES])
{
  size_t i;
...
static void unpack_pk(polyvec *pk,
                      uint8_t seed[KYBER1024_SYMBYTES],
                      const uint8_t packedpk[KYBER1024_INDCPA_PUBLICKEYBYTES])
{
  size_t i;
...
**************************************************/
static void pack_sk(uint8_t r[KYBER1024_INDCPA_SECRETKEYBYTES], polyvec *sk)
{
...
static void unpack_sk(polyvec *sk,
                      const uint8_t packedsk[KYBER1024_INDCPA_SECRETKEYBYTES])
{
...
**************************************************/
static void pack_ciphertext(uint8_t r[KYBER1024_INDCPA_BYTES],
                            polyvec *b,
                            poly *v)
{
  polyvec_compress(r, b);
  poly_compress(r+KYBER1024_POLYVECCOMPRESSEDBYTES, v);
}
                              ...
                              const uint8_t c[KYBER1024_INDCPA_BYTES])
{
  ...
  poly_decompress(v, c+KYBER1024_POLYVECCOMPRESSEDBYTES);
}
...
*                                     is generated
**************************************************/
#define GEN_MATRIX_NBLOCKS ((12*KYBER1024_N/8*(1 << 12)/KYBER1024_Q \
...
void gen_matrix(polyvec *a, const uint8_t seed[KYBER1024_SYMBYTES], int transposed)
{
  ...
  for(i=0;i<KYBER1024_K;i++) {
    for(j=0;j<KYBER1024_K;j++) {
      ...
      ctr = rej_uniform(a[i].vec[j].coeffs, KYBER1024_N, buf, buflen);

      while(ctr < KYBER1024_N) {
        off = buflen % 3;
        for(k = 0; k < off; k++)
          buf[k] = buf[buflen - off + k];
        xof_squeezeblocks(buf + off, 1, &state);
        buflen = off + XOF_BLOCKBYTES;
        ctr += rej_uniform(a[i].vec[j].coeffs + ctr, KYBER1024_N - ctr, buf, buflen);
      }
    }
  }
}
...
*                             (of length KYBER1024_INDCPA_PUBLICKEYBYTES bytes)
*              - uint8_t *sk: pointer to output private key
                              (of length KYBER1024_INDCPA_SECRETKEYBYTES bytes)
**************************************************/
                    ...
                    uint8_t sk[KYBER1024_INDCPA_SECRETKEYBYTES])
{
  ...
  const uint8_t *noiseseed = buf+KYBER1024_SYMBYTES;
  uint8_t nonce = 0;
  polyvec a[KYBER1024_K], e, pkpv, skpv;

  kyber_randombytes(buf, KYBER1024_SYMBYTES);
  hash_g(buf, buf, KYBER1024_SYMBYTES);
...
*                                      (of length KYBER1024_INDCPA_BYTES bytes)
*              - const uint8_t *m:     pointer to input message
*                                      (of length KYBER1024_INDCPA_MSGBYTES bytes)
*              - const uint8_t *pk:    pointer to input public key
*                                      (of length KYBER1024_INDCPA_PUBLICKEYBYTES)
*              - const uint8_t *coins: pointer to input random coins
*                                      used as seed (of length KYBER1024_SYMBYTES)
                ...
                const uint8_t coins[KYBER1024_SYMBYTES])
{
  ...
  uint8_t seed[KYBER1024_SYMBYTES];
  uint8_t nonce = 0;
  polyvec sp, pkpv, ep, at[KYBER1024_K], bp;
  poly v, k, epp;
...
*                                   (of length KYBER1024_INDCPA_MSGBYTES)
*              - const uint8_t *c:  pointer to input ciphertext
*                                   (of length KYBER1024_INDCPA_BYTES)
*              - const uint8_t *sk: pointer to input secret key
*                                   (of length KYBER1024_INDCPA_SECRETKEYBYTES)
**************************************************/
